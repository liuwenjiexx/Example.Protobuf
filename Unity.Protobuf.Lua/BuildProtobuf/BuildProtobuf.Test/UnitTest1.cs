...
namespace BuildProtobuf.Test
{
    ...
    public class UnitTest1
    {
        ...
        public void SC_Pattern()
        {
            ...
            Assert.IsFalse(regex.Match("SC_abc").Groups["cs"].Success);
        ...
        [TestMethod]
        public void Request_Response_Pattern()
        {
            Regex regex = new Regex("(^|\\.)(?<name>[^\\.]+?)((?<cs>Request)|(?<sc>Response))?$");

            Assert.AreEqual("abc", regex.Match("abc").Groups["name"].Value);
            Assert.AreEqual("abc", regex.Match("ns.abc").Groups["name"].Value);
